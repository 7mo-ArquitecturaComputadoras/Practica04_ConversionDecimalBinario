...
\begin{figure}[H]
    ...
    \resizebox{0.72\textwidth}{!}{%
        \begin{tikzpicture}[
            ...
            \node[process,   below=0.8cm of loop]    (extraer) {SHL EAX,1 $\to$ bit cae en CF};
            \node[process,   below=0.8cm of extraer] (setc)    {SETC BL $\to$ BL = bit (0 ó 1)};
            \node[process,   below=0.8cm of setc]    (escribir){BL + \texttt{'0'} $\to$ [EDI], EDI++, ECX$--$};
            ...
            % Línea de retorno del Bucle (viaja por el margen izquierdo)
...
decimalABinario PROC numero:DWORD
    ...
    jz   caso_base              ; Si es cero, caso especial
...
caso_base:
    mov  BYTE PTR [edi], '0'    ; Escribimos '0' directamente
    inc  edi                    ; Avanzamos el puntero
...
\begin{itemize}
    ...
    \item \textbf{ECX} (\textit{Extended Counter Register}): actúa como contador del ciclo. Se inicializa con la posición del MSB más uno, y se decrementa en cada iteración hasta llegar a cero.
    \item \textbf{EDX} (\textit{Extended Data Register}): almacena temporalmente la cantidad de lugares a desplazar para la alineación inicial del número (\texttt{31 - posición\_MSB}).
    \item \textbf{EBX} / \textbf{BL}: \texttt{BL} (byte bajo de \texttt{EBX}) recibe el bit extraído vía \texttt{SETC} y lo convierte al carácter de texto correspondiente antes de escribirlo en el buffer.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{imagenes/memoria_buffer.png}
    \caption{Ventana Memory de Visual Studio: buffer con el resultado \texttt{"101101"} tras convertir 45.}
    \label{fig:memoria_buffer}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{imagenes/consola_resultado.png}
    \caption{Salida en consola: el usuario ingresa 45 y el programa devuelve \texttt{101101}.}
...
\url{https://github.com/7mo-ArquitecturaComputadoras/Practica04_ConversionDecimalBinario}
...
\begin{itemize}
    ...
    \item La \textbf{interfaz entre C++ y ensamblador} mediante \texttt{extern "C"} ilustra cómo los lenguajes de alto nivel pueden delegar operaciones críticas de bajo nivel a rutinas escritas directamente en ensamblador, devolviendo resultados a través de registros o punteros.
